 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */let tk=new Map;function tO(e,t){let n=tD(e);tA(n,t),function(e,t){let n=(!tL&&"BroadcastChannel"in self&&((tL=new BroadcastChannel("[Firebase] FID Change")).onmessage=e=>{tA(e.data.key,e.data.fid)}),tL);n&&n.postMessage({key:e,fid:t}),0===tk.size&&tL&&(tL.close(),tL=null)}(n,t)}function tA(e,t){let n=tk.get(e);if(n)for(let e of n)e(t)}let tL=null,tN="firebase-installations-store",tB=null;function tP(){return tB||(tB=ts("firebase-installations-database",1,{upgrade:(e,t)=>{0===t&&e.createObjectStore(tN)}})),tB}async function tM(e,t){let n=tD(e),r=await tP(),a=r.transaction(tN,"readwrite"),i=a.objectStore(tN),o=await i.get(n);return await i.put(t,n),await a.done,o&&o.fid===t.fid||tO(e,t.fid),t}async function tj(e){let t=tD(e),n=await tP(),r=n.transaction(tN,"readwrite");await r.objectStore(tN).delete(t),await r.done}async function tR(e,t){let n=tD(e),r=await tP(),a=r.transaction(tN,"readwrite"),i=a.objectStore(tN),o=await i.get(n),s=t(o);return void 0===s?await i.delete(n):await i.put(s,n),await a.done,s&&(!o||o.fid!==s.fid)&&tO(e,s.fid),s}/**
 * @license
 * Copyright 2019 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
